...
The first thing i would do i check if any new dependencies are flagged as dangerous on registries such as npm or nuget. If not then I would create an agent that goes through the code changes that checks if there are any properly written tests that it can run, if not then it should look through the PR description and codebase for how the project should work and ask the user what the intended behaviour should be . Then I would have another agent scan the pull request for any security-sensitive operations are definite red flags such as SQL injection. If the project has cloud infrastructure then Sentinel should check for permission flags and ensure that they are being honoured within the code. 
