...
I noticed when reading the readme is that Sentinel does not keep track of security regressions; only whether or not the current PR is a security risk. I believe it would be useful for agents to check whether the PR is making project security worse compared to pass iterations. This is worth exploring because A PR can be free of security risk without actually making the project any more secure.